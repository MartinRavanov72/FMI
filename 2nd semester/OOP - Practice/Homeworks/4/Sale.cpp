...
Sale::Sale(double _amount, Product* _product, User* _giver, User* _receiver){
...
bool Sale::start(){
	if(this->getGiver()->getMoney() < this->getAmount() || !this->getGiver()->getLoginStatus() || !this->getReceiver()->getLoginStatus()){
		...
		cout << "The payment for " << this->getProduct()->getName() << " for " << this->getAmount() << "$ is successful. Buyer: " << this->getGiver()->getName() << ". Seller: " << this->getReceiver()->getName() <<endl;
		return true;
	}
